  ...
  export default function AddSection(props) {
    ...
    useEffect(() => {
      ...
      async function handleSectionSubmit(values) {
        ...
        try {
          setisLoading(true);
          let response = await api.post(`${config.api}/courses/${courseData.id}/sections`, values);
      ...
      return <>
          ...
          <div className="alert alert-danger">{formik.errors.name}</div>
            ...
            />
          {formik.errors && formik.errors?.section_num && formik.touched?.section_num ? (
              ...
              formik.values.lessons.map((lesson, lessonIndex) => (
                <div className="lesson" key={lessonIndex}>
                  ...
                  onBlur={formik.handleBlur}
                    value={lesson.name}
                  />
                  {formik.errors && formik.errors.lessons && formik.errors.lessons[lessonIndex]?.name &&
                  formik.touched?.lessons?.[lessonIndex]?.name ? (
                  <div className="alert alert-danger">{formik.errors.lessons[lessonIndex].name}</div>
                  ) : null}
                  <textarea
                  ...
                  />
                  {formik.errors && formik.errors?.lessons && formik.errors.lessons[lessonIndex]?.content &&
                  formik.touched?.lessons?.[lessonIndex]?.content ? (
                  <div className="alert alert-danger">{formik.errors.lessons[lessonIndex].content}</div>
                  ) : null}
                  <input
                  ...
                  <div className="alert alert-danger">{formik.errors.lessons[lessonIndex].lesson_num}</div>
                  ) : null}
                  <input
            ...
            <button
              ...
              onClick={() => {
                let newSections = { ...formik.values };
                newSections.lessons.push({ name: '', content: '', lesson_num: '', completed: false });
                formik.setFieldValue('lessons', newSections.lessons);
              }}
            >
              Add Lesson
      ...
      <p></p>
          ...
          <button  className='btn btn-primary btn-lg btn-block' disabled={!formik.dirty && formik.isValid} type="submit" onSubmit={handleSectionSubmit} >create Section</button>}
          <Toaster/>
          </form>
      </div>
       </>
